...
function TypeWriterText() {
  const text = "كل ما يحتاجه حيوانك… في مكان واحد";
  ...
  return (
    <p className="text-white text-lg md:text-xl font-black leading-snug">
      {displayedText}
...
export default function InformationPage() {
  return (
    <section
      dir="rtl"
      className='w-full min-h-screen flex items-center justify-center bg-white dark:bg-[#101828] p-4 transition-colors duration-500 relative'
    >
          ...
          <div className="flex flex-col items-center text-center order-2 md:order-1 w-full">
            ...
            <div className="flex flex-row flex-wrap justify-center gap-4">
              <Button
                as={Link}
                href='/login'
                className="bg-mainColor text-white px-12 py-7 rounded-full text-2xl font-black shadow-lg hover:scale-105 transition-transform cursor-pointer"
              >
                تسجيل الدخول
              </Button>
            </div>
      ...
      <div className="fixed bottom-20 right-10 bg-mainColor p-4 rounded-[40px] flex items-center gap-6 shadow-[0_20px_50px_rgba(0,0,0,0.3)] z-100 hover:scale-105 transition-transform duration-300 border-2 border-white/10">

        <div className="text-right flex flex-col justify-center order-2">
          <p className="text-white text-sm font-bold opacity-80 mb-1">
            منصة كاجو
          </p>
        ...
        <div className="relative order-1">
          ...
          <img
            src={notebook}
            alt="Notebook"
            className="w-32 h-full rounded-4xl object-contain relative z-10 drop-shadow-xl"
          />
        </div>
